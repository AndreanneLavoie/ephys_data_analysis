function spikes = create_spikes(kilosort_data_path)
...
raw_path = dir([exp_name '*.*']);
exp_ephys_ind = strfind(raw_path.name, '_ephys'); %index of '_' in _ephys 
date = raw_path(1).name((exp_ephys_ind + 6):(exp_ephys_ind + 12)); %e.g. '_190223'
vspara_path = [general_path 'VSpara\'];

spikes.raw_data_path = [raw_path.folder '\' raw_path.name];

tuning_param_ind = strfind(exp_name,'-');
tuning_param_ind = tuning_param_ind(4)+1;
tuning = exp_name(tuning_param_ind:tuning_param_ind+1);
...
if ~strcmp(tuning(1:2), 'or') %for sf and tf tuning
    vspara_filename = ['20trk' date(6:7) date(4:5) date(2:3) '_' exp_name];
    ...
    if isfile([vspara_path vspara_filename '.eye']) % a single vspara file for recording
        ...
        if any(startsWith(file_list,string(vspara_filename)))%list folder names for all the recodings starts with 'vgt' or 'c57'
            ...
            f=msgbox(['no vs file exist for ' exp_name])
            return
        end
    ...
    if isfile([vspara_path vspara_filename '.csv'])   
        ...
        if any(startsWith(file_list,string(vspara_filename)))
            ...
            if length(vs_files_ind)>1
                ...
                spikes.vs_params = csvread([vspara_path vspara_files{1,1}]);
            end
        else 
            f=msgbox(['no vs file exist for ' exp_name])
            return 
...
%         %%FOR RECORDINGS THAT LED CONDITION IS NOT SAVED IN vs_param COLUMN 17 yet        
%         vs_params_low = csvread([vspara_path vspara_filename 'low_20' date(2:end) '.csv']);
%         %vs_params_med = csvread([vspara_path vspara_filename 'med_20' date(2:end) '.csv']);
%         vs_params_high = csvread([vspara_path vspara_filename 'high_20' date(2:end) '.csv']);
%         %add extra column to mark which power was used; low = 1; med = 2;high = 3 
%         vs_params_low(:, end+1) = 1; 
%        % vs_params_med(:, end+1) = 2; 
%         vs_params_high(:, end+1) = 3; 
%         spikes.vs_params = [vs_params_low; vs_params_high]; %[vs_params_low; vs_params_med; vs_params_high];
    end
end


% read in info.rhd 
spikes.intan_info=read_Intan_RHD2000_file_ana('info.rhd',[spikes.raw_data_path '\']);
amplifier_sample_rate= spikes.intan_info.frequency_parameters.amplifier_sample_rate; %frequency_parameters.amplifier_sample_rate;
...
spikes.vstiming = extract_signal_time('time.dat', 'board-ADC-00.dat', spikes.raw_data_path, threshold,digfilter_winT_fast, amplifier_sample_rate);
spikes.trigtiming= extract_signal_time('time.dat', 'board-ADC-01.dat', spikes.raw_data_path, threshold,digfilter_winT_fast, amplifier_sample_rate); %visual stim trigger times
%ERROR CHECK for vs time calculation 
if any(abs(spikes.vstiming(:,3)-spikes.vs_params(1,7)/1000)>spikes.vs_params(1,7)*percT_thres/1000)
   fprintf ('ERROR in vstiming calculation, vs period longer than set\n');
end
%ERROR CHECK for trigger time calculation
if any(abs(spikes.trigtiming(:,3)-preset_trigdur)>preset_trigdur*percT_thres)
   fprintf ('ERROR in trigtiming calculation, trigger period longer than set\n');
end
...
if  sum(spikes.vs_params(:,17)) > 0
    ...
    if abs(spikes.vs_params(1,16)-1)<10e-8
        ...
        spikes.led_abstime = extract_signal_time('time.dat', 'board-ADC-02.dat', spikes.raw_data_path, threshold, digfilter_winT_slow, amplifier_sample_rate); %visual stim trigger times
    else
        spikes.led_abstime = extract_signal_time('time.dat', 'board-ADC-02.dat', spikes.raw_data_path, threshold, digfilter_winT_fast, amplifier_sample_rate); %visual stim trigger times
    end
...
% THIS PART OF CODE WORKS, JUST NEED TO ADD PROBE NAME AS INPUT TO FUNCTION    
%     load(['E:\extarcellular\Andreanne\chanMaps\' probe])
%     spikes.channel = spikes.assigns;
%     spikes.depth = spikes.assigns;  
